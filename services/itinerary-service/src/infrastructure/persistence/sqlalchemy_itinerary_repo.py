...
class SqlAlchemyItineraryRepository(ItineraryRepositoryPort, OutboxRepositoryPort):
    ...
    async def find_by_id(self, itinerary_id: uuid.UUID) -> Optional[Itinerary]:
        session: Session = self.session_factory()
        try:
            model = session.query(ItineraryModel).filter(ItineraryModel.id == str(itinerary_id)).first()
            if model:
    ...
    async def list_all(self) -> List[Itinerary]:
        session: Session = self.session_factory()
        try:
            models = session.query(ItineraryModel).order_by(ItineraryModel.created_at.desc()).all()
    ...
    async def get_pending_events(self, limit: int = 20) -> List[OutboxEvent]:
        session: Session = self.session_factory()
        try:
            models = session.query(OutboxEventModel)\
    ...
    async def mark_as_published(self, event_id: uuid.UUID) -> None:
        session: Session = self.session_factory()
        try:
            model = session.query(OutboxEventModel).filter(OutboxEventModel.id == str(event_id)).first()
            if model:
