...
from src.infrastructure.api.schemas import CreateItineraryRequest, ItineraryResponse, HealthCheckResponse
...
def create_itinerary_router(
    ...
    async def create_itinerary(payload: CreateItineraryRequest, request: Request):
        ...
        trace_id = getattr(request.state, "trace_id", "00000000000000000000000000000000")
    ...
    async def healthcheck():
        ...
        try:
            ...
            session.execute("SELECT 1" if "sqlite" in session.bind.dialect.name else "SELECT 1")
            session.close()
        except Exception:
            db_ok = False
