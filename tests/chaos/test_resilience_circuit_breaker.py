...
# Ajustar PYTHONPATH
sys.path.insert(0, os.path.abspath(os.path.join(os.path.dirname(__file__), "../../services/airport-service")))
...
async def run_resilience_demo():
    ...
    cb = CircuitBreaker(failure_threshold=3, recovery_timeout=2.0, name="ChaosTestBreaker")
    ...
    for i in range(1, 4):
        ...
        print(f"  -> Intento #{i}: Fallo registrado. Tiempo: {elapsed:.2f}ms. Fallas acumuladas: {cb.failure_count}/{cb.failure_threshold}")

    print(f"\n[PASO 3] Verificando apertura del circuito...")
    print(f"  -> Estado actual del Circuito: {cb.state.value}")
    assert cb.state == CircuitState.OPEN, "El circuito debió dispararse a OPEN tras 3 fallos"
    ...
    for i in range(1, 6):
        ...
        print(f"  -> Petición #{i} (Circuito OPEN): Retornó {len(airports)} aeropuertos de fallback en {elapsed:.3f}ms")

    avg_time = sum(times) / len(times)
    print(f"\n  [EVIDENCIA] Tiempo promedio de respuesta en OPEN: {avg_time:.3f}ms (Menos de 5ms)")
    assert avg_time < 5.0, f"El tiempo promedio debe ser menor a 5ms, obtenido: {avg_time}ms"

    # 4. Probar recuperación a HALF-OPEN
    print("\n[PASO 5] Esperando ventana de recuperación (2.0 segundos)...")
    await asyncio.sleep(2.1)

    print(f"  -> Evaluando transición de prueba...")
    # Ejecutamos una llamada que forzará HALF-OPEN
    # Al fallar nuevamente regresará a OPEN
    airports = await adapter.get_all_airports()
    print(f"  -> Estado posterior a prueba: {cb.state.value} (Retornó fallback sin saturar)")
